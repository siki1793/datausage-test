...
var morning_start = moment("2015-05-26T07:00:00.000Z");  // creating to a moment object
var morning_end = moment("2015-05-26T12:00:00.000Z");
...
var afternoon_start = moment("2015-05-26T12:00:00.000Z"); // can be entered as per local format with offset
var afternoon_end = moment("2015-05-26T16:00:00.000Z");
...
var evening_start = moment("2015-05-26T16:00:00.000Z");
var evening_end = moment("2015-05-26T20:00:00.000Z");
...
var night_start = moment("2015-05-26T20:00:00.000Z");
var night_end = moment("2015-05-26T23:59:00.000Z");
...
var earlymorning_start = moment("2015-05-26T00:00:00.000Z");
var earlymorning_end = moment("2015-05-26T07:00:00.000Z");
...
allUserQuery.post(function(req,res){
	...
	var getData = function(session){
		...
		*/
		...
		case "Morning" : 
							...
							console.log(sessionStartHours+" "+sessionStartMinutes);
							console.log(sessionEndHours+" "+sessionEndMinutes);
							break;
		case "AfterNoon" : 
		...
		getdata.aggregate([{$unwind : '$totalData' },{$match : { "totalData.date" : { "$gte" : fromDate,"$lte" : toDate}}},
		               	                 
		             	                   {$project :{ "time" : { "$add": [
		             	                                                    { "$hour": "$totalData.date" },
		             	                        	                        { "$divide": [{ "$minute": "$totalData.date" }, 60] }
		             	                        	                    ]},"totalData.date" : 1,"totalData.wifiTX":1,"totalData.wifiRX" : 1,"totalData.cellTX" : 1, "totalData.cellRX" :1, "_id" : 0  } },
		             	                   {$match :  { "$and": [
		             	     ...
		             	     { $group: {_id : null,
		             	     timeValue: {$avg:"$time"},
		             		 totalTxWifi : { $avg : "$totalData.wifiTX"},
		             		 totalRxWifi : { $avg : "$totalData.wifiRX"},
		             		 totalTxCell : { $avg : "$totalData.cellTX"},
		             		 totalRxCell : { $avg : "$totalData.cellRX"} } 
		              } ],function(err,docs){
		             		if(err){
		             			console.log(err);
		             			return 0;
		             			...
		             			//console.log(docs);
		             				...
		             				if(docs.length == 0){	
		             					...
		             					for(var i =0 ; i < docs.length;i++){ // this for loop has no significance, any way length would be 1  
		             					    ...
		             					    jsondata= givejson(session,"timeValue",docs[i].timeValue);
		             					    session_data.push(jsondata);
		             					    //fs.appendFileSync(jsonFile, JSON.stringify(jsondata));
		             					    }
		             					}
