...
var afternoon_start = moment("2015-05-26T11:00:00.000+0530"); // can be entered as per local format with offset
		...
		*/
		...
		getdata.aggregate([{$unwind : '$data' },{$match : { "data.date" : { "$gte" : fromDate,"$lte" : toDate}}},
		             	                   ...
		             	                   {$project :{ "time" : { "$add": [
		             	                        	                    ...
		             	                        	                    ]}, "data.date" : 1,"data.txwifi":1,"data.rxwifi" : 1,"data.txcell" : 1, "data.rxcell" :1,"IMEI" : 1,"data.server_synctime":1, "_id" : 0  } },
		             	                   {$match :  { "$and": [
				...
				//{ $group: {_id : null,
		              ...
		              //}
						...
						for(var i=0; i<docs.length;i++){
							...
							//console.log(docs[i]);	
							if(docs[i].data.txwifi < 0 || docs[i].data.rxwifi <0 || docs[i].data.txcell <0 || docs[i].data.rxcell <0 )
							{
								test_docs.push(docs[i]);
							}
						 }
		             			
		             			console.log(test_docs);		
		             		  }
